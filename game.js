...
const SPAWN_MIN = 3000, SPAWN_MAX = 3500;   // คูลดาวน์เกิดมอนใหม่ ~3 วินาที
const FLEE_MS = 45000;
...
// TIER_LABEL/ORDER/EMOJI/LEVEL, TYPE_EMOJI/TH ย้ายไป content.js แล้ว (import ด้านบน)
function typeBadges(types) {
  return types.map(t => `<span class="badge t-${t}">${TYPE_EMOJI[t] || ''} ${TYPE_TH[t] || t}</span>`).join('');
}

// TIER_WEIGHTS ย้ายไป content.js แล้ว (import ด้านบน)
...
// WEATHERS/NIGHT_BOOST/DAY_BOOST ย้ายไป content.js แล้ว (import ด้านบน)
...
// Z_MOVES ย้ายไป content.js แล้ว (import ด้านบน)
...
const BP_SHOP = [
  ...
  { id: 'bp_abilitypatch', name: 'Ability Patch', emoji: '🔬', img: 'ability-capsule', cost: 10000, act: () => { state.abilityPatches = (state.abilityPatches || 0) + 1; return '🔬 +1 Ability Patch (ปลดล็อกความสามารถซ่อนที่หน้าโปเกมอน)'; } },
...
function rollHit(move, atkHeld, defHeld, defAbility) {   // เช็คว่าท่านี้แม่นเป้าไหม (คิดไอเทม Wide Lens/Bright Powder + Sand Veil ด้วยถ้ามี)
  ...
  if (defAbility && defAbility.fx === 'sandveil' && getWeather(state.region) === 'sand') acc -= 15;
...
function newSave() {
  return {
    ...
    bpBought: {},        // {itemId: count} จำนวนที่แลกไปแล้ว (ใช้กันของ limited) — รีเซ็ตทุก 2 วัน
    bpShopResetAt: 0,    // เวลารีเซ็ตร้าน BP รอบถัดไป
    abilityPatches: 0,   // Ability Patch สำหรับปลดล็อกความสามารถซ่อน
...
function save() {
  if (state) { state.lastSeen = Date.now(); persistSpawn(); }
...
function clearSpawn() {
  currentSpawn = null; state.savedSpawn = null;
...
function openIvRerollModal(uid) {
  ...
  const rows = keys.map(k => {
    const locked = _ivLocks.has(k);
    return `<div class="iv-bar-wrap">
      <span class="lbl">${STAT_LABEL[k]}</span>
      <div class="iv-bar"><div style="width:${ind.iv[k] / 31 * 100}%"></div></div>
      <span class="val">${ind.iv[k]}/31</span>
      <button class="mini-btn" data-ivlock="${k}" style="padding:4px 9px;font-size:12px;${locked ? '' : 'opacity:.45;background:var(--btn-ghost-bg)'}">${locked ? '🔒' : '🔓'}</button>
    </div>`;
  }).join('');
  const cost = ivRerollCost();
  $('#modalBox').innerHTML = `
    <h3 style="font-size:18px;margin:2px 0 6px">🎲 สุ่มค่า IV ใหม่</h3>
    <div class="sr-sub" style="margin-bottom:10px">${MON_BY_ID[ind.id].name} · ล็อกช่องที่พอใจไว้ (ยิ่งล็อกยิ่งแพง) · แต่ละช่องมีโอกาส 1/100 ได้ IV เต็ม (31)</div>
    <div style="text-align:left;margin-bottom:12px;display:flex;flex-direction:column;gap:6px">${rows}</div>
    <div style="font-weight:800;margin-bottom:10px">ราคา: <span style="color:var(--accent)">${cost.toLocaleString()}🪙</span> <span class="sr-sub">(พื้นฐาน 100,000 + ล็อก ${_ivLocks.size} ช่อง)</span></div>
    <div class="modal-actions">
      <button class="btn-primary" id="ivDo" ${(state.coins < cost || _ivLocks.size >= 6) ? 'disabled' : ''}>🎲 สุ่มใหม่ (${state.coins.toLocaleString()}🪙)</button>
      <button class="btn-ghost" id="ivBack">กลับ</button>
    </div>`;
  openModal();
  $('#modalBox').querySelectorAll('[data-ivlock]').forEach(b => b.onclick = () => {
    const k = b.dataset.ivlock; _ivLocks.has(k) ? _ivLocks.delete(k) : _ivLocks.add(k); openIvRerollModal(uid);
  });
  $('#ivBack').onclick = () => openIndividualModal(uid);
  $('#ivDo').onclick = () => {
    if (_ivLocks.size >= 6) { toast('ล็อกครบทุกช่องแล้ว ไม่มีอะไรให้สุ่ม', ''); return; }
    const c = ivRerollCost();
    if (!spend(c)) return;
    keys.forEach(k => { if (!_ivLocks.has(k)) ind.iv[k] = Math.random() < IV_MAX_CHANCE ? 31 : rand(0, 30); });
    save(); renderTopbar(); checkAchievements();
    toast(`🎲 สุ่ม IV ใหม่! IV รวม ${ivPercent(ind)}%`, 'good');
...
function openIndividualModal(uid) {
  ...
  $('#modalBox').innerHTML = `
    ...
    <div style="margin-bottom:8px">
      ...
      ${abilityFor(ind.id, ind) ? `<span class="pill" title="${abilityFor(ind.id, ind).desc}">🧬 ${abilityFor(ind.id, ind).name}${ind.hidden ? ' (ซ่อน)' : ''}</span>` : ''}
    </div>
    ...
    <div class="modal-actions">
      ...
      <button class="btn-ghost" id="mIvReroll">🎲 สุ่ม IV ใหม่</button>
      ${(hiddenAbilityFor(ind.id) && !ind.hidden) ? `<button class="btn-primary" id="mPatch" ${(state.abilityPatches || 0) <= 0 ? 'disabled' : ''}>🔬 Ability Patch (${state.abilityPatches || 0})</button>` : ''}
  ...
  const pc = $('#mPatch'); if (pc) pc.onclick = () => {
    const ha = hiddenAbilityFor(ind.id); if (!ha) return;
    if ((state.abilityPatches || 0) <= 0) { toast('❌ ไม่มี Ability Patch (ซื้อที่ร้าน BP)', 'bad'); return; }
    if (!confirmAction(`ใช้ Ability Patch เปลี่ยน ${MON_BY_ID[ind.id].name} เป็นความสามารถซ่อน "${ha.name}"?`)) return;
    state.abilityPatches--; ind.hidden = true; save();
    toast(`🔬 ${MON_BY_ID[ind.id].name} → ${ha.name} (ความสามารถซ่อน)!`, 'good');
...
const BP_SHOP_CD = 2 * 86400000;   // ร้าน BP รีเซ็ตลิมิตทุก 2 วัน
function ensureBpShopFresh() {
  if (Date.now() >= (state.bpShopResetAt || 0)) { state.bpBought = {}; state.bpShopResetAt = Date.now() + BP_SHOP_CD; save(); }
}
function renderBpShop() {
  const box = $('#bpShopBox'); if (!box) return;
  ensureBpShopFresh();
  const left = (state.bpShopResetAt || 0) - Date.now();
  const refreshTxt = left > 0 ? ` · รีเซ็ตอีก ${Math.floor(left / 3600000)}ชม` : '';
  box.innerHTML = `<div class="dex-stats">🎖️ Battle Points: ${state.battlePoints || 0} (ได้จากชนะยิม/บอส)${refreshTxt}</div>` +
    BP_SHOP.map(it => {
...
function renderBattleGuide() {
  ...
  // ความสามารถประจำตัวที่ "มีผลในการต่อสู้" (จากทั้งหมด ~224 แบบ แต่ละตัวใช้ของจริง) — โชว์เฉพาะที่มีกลไก
  const fxAbilities = Object.values(ABILITY_DEFS)
    .filter(ab => ab.fx || ab.boostType || ab.onHitStatus || ab.immuneType)
    .filter((ab, i, arr) => arr.findIndex(x => x.name === ab.name) === i)
    .sort((a, b) => a.name.localeCompare(b.name));
  const abilityRows = fxAbilities.map(ab =>
    `<div class="preset-row"><span class="pr-name"><b>🧬 ${ab.name}</b></span>
      <span style="font-size:11px;color:var(--muted);text-align:right;max-width:60%">${ab.desc}</span></div>`).join('');
  box.innerHTML = `
    ...
    <div style="font-size:12px;font-weight:700;margin-bottom:6px">🧬 ความสามารถประจำตัว (Passive) — แต่ละสายพันธุ์มีของตัวเองเหมือนเกมจริง</div>
    <div class="sr-sub" style="margin-bottom:6px">ที่มีผลในการต่อสู้ตอนนี้:</div>
...
function applyStatFx(mv, casterStages, targetStages, casterName, targetName, casterAbility, targetAbility) {
  ...
  if (casterAbility && casterAbility.fx === 'serenegrace') chance = Math.min(1, chance * 2);   // Serene Grace: โอกาสเอฟเฟกต์ท่าตัวเอง ×2
  if (Math.random() > chance) return '';
  const isSelf = fx.target === 'self';
  if (!isSelf && fx.delta < 0 && targetAbility && targetAbility.fx === 'clearbody') return '';   // Clear Body บล็อกท่าที่ลดสเตตัสจากศัตรู
...
// ===== Ability ประจำตัว — ใช้ความสามารถจริงของแต่ละสายพันธุ์ (abilities-data.js) ครบ 1025 ตัว =====
// ความสามารถประจำตัวจริงของแต่ละสายพันธุ์ (จาก abilities-data.js) — ถ้าไม่มีในดิกชันนารีสร้างชื่อ/คำอธิบายกลางให้
const _abilityKeyCache = {};
function abilityByKey(key) {
  if (!key) return null;
  if (_abilityKeyCache[key] !== undefined) return _abilityKeyCache[key];
  let ab = ABILITY_DEFS[key];
  if (!ab) {   // มีชื่อความสามารถแต่ยังไม่มีคำอธิบาย — สร้างชื่อสวยๆ จาก identifier
    const nm = key.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
    ab = { name: nm, desc: 'ความสามารถประจำตัว' };
  }
  _abilityKeyCache[key] = ab;
  return ab;
}
// ความสามารถของสายพันธุ์ — ถ้าตัวนั้นใช้ Ability Patch (ind.hidden) จะได้ความสามารถซ่อนแทน
function abilityFor(id, ind) {
  if (ind && ind.hidden && HIDDEN_ABILITY[id] && HIDDEN_ABILITY[id] !== SPECIES_ABILITY[id]) return abilityByKey(HIDDEN_ABILITY[id]);
  return abilityByKey(SPECIES_ABILITY[id]);
}
// ความสามารถซ่อนของสายพันธุ์ (ต่างจากปกติ) — คืน null ถ้าไม่มี/เหมือนปกติ
function hiddenAbilityFor(id) {
  const h = HIDDEN_ABILITY[id];
  return (h && h !== SPECIES_ABILITY[id]) ? abilityByKey(h) : null;
}
...
function applyIntimidate(holderSide, b) {
  const holderInd = holderSide === 'player' ? b.team[b.activeIdx].ind : null;
  const holderId = holderSide === 'player' ? b.team[b.activeIdx].ind.id : b.foeMon.id;
  const ab = abilityFor(holderId, holderInd);
  if (!ab || ab.fx !== 'intimidate') return '';
  const targetInd = holderSide === 'player' ? null : b.team[b.activeIdx].ind;
  const targetId = holderSide === 'player' ? b.foeMon.id : b.team[b.activeIdx].ind.id;
  const targetAbility = abilityFor(targetId, targetInd);
  const targetName = holderSide === 'player' ? (b.foeDisplayName || b.foeMon.name) : activeMonView(b.team[b.activeIdx]).name;
  if (targetAbility && targetAbility.fx === 'clearbody') return ` · 😤 Intimidate! แต่ ${targetName} มี ${targetAbility.name} ต้านไว้`;
...
function applyRegenerator(member) {
  const ab = abilityFor(member.ind.id, member.ind);
  if (!ab || ab.fx !== 'regenerator' || member.hp <= 0) return '';
...
function tryInflict(move, target, targetTypes, name, targetAbility, casterAbility) {
  ...
  if (targetAbility && targetAbility.fx === 'insomnia' && ts.s === 'sleep') return '';
  const chance = (casterAbility && casterAbility.fx === 'serenegrace') ? Math.min(1, ts.c * 2) : ts.c;
...
function foeTurn(b) {
  ...
  if (!rollHit(mv, b.foeHeld, active.ind.held, abilityFor(active.ind.id, active.ind))) {                     // ท่าศัตรูพลาดเป้า
    b.msg += ` · ${foeName} ใช้ ${mv.name}! แต่พลาดเป้า... 💨`;
    return;
  }
  const atkAbility = abilityFor(b.foeMon.id), defAbility = abilityFor(active.ind.id, active.ind);
...
function endRound(b) {
  ...
  if (a && a.hp > 0 && a.hp < a.maxHp && w === WEATHERS.snow) {   // Ice Body ฟื้นตอนหิมะโปรย
    const ab = abilityFor(a.ind.id, a.ind);
    if (ab && ab.fx === 'icebody') { const heal = Math.max(1, Math.floor(a.maxHp / 16)); a.hp = Math.min(a.maxHp, a.hp + heal); b.msg += ` · ❄️ Ice Body! ฟื้น ${heal}`; }
  }
  ...
  if (!b.over && b.foeHp > 0 && b.foeHp < b.foeMaxHp && w === WEATHERS.snow) {   // Ice Body ฝั่งศัตรู
    ...
    if (ab && ab.fx === 'icebody') { const heal = Math.max(1, Math.floor(b.foeMaxHp / 16)); b.foeHp = Math.min(b.foeMaxHp, b.foeHp + heal); b.msg += ` · ❄️ Ice Body! ${b.foeDisplayName || b.foeMon.name} ฟื้น ${heal}`; if (b.mode === 'wild' && currentSpawn) currentSpawn.hp = b.foeHp; }
  }
...
function battleAttack(moveIdx) {
  ...
  if (b.isRaid) foeFirst = false;   // World Boss: ฝั่งเทรนเนอร์ตีก่อนเสมอ
  ...
  function playerHalf() {   // คืนค่า true ถ้าเทิร์นจบทันที (KO/ป่าอ่อนแรงสุดขีด) ไม่ต้องรอศัตรูตอบโต้
    ...
    const atkAbility = abilityFor(active.ind.id, active.ind), defAbility = abilityFor(b.foeMon.id);
    ...
    if (active.ind.held === 'shell-bell' && !b.isRaid) {   // Shell Bell: ฟื้น HP 1/8 ของดาเมจที่ทำ (World Boss ดูดเลือดไม่ขึ้น)
      const heal = Math.max(1, Math.floor(dmg / 8));
      active.hp = Math.min(active.maxHp, active.hp + heal);
      b.msg += ` · 🐚 ฟื้น ${heal}`;
    }
...
function makeRaidFoeDef(mon) {
  ...
  // บอสเก่งกว่าปกติ 3 เท่า (ATK/SpA/SPD) แต่ DEF/SpD แค่ 2 เท่า (พอตีทะลุได้)
  const stats = { atk: Math.floor(base.atk * 3), def: Math.floor(base.def * 2), spatk: Math.floor(base.spatk * 3), spdef: Math.floor(base.spdef * 2), spd: Math.floor(base.spd * 3) };
...
function init() {
  ...
  preloadMascots();
  checkAchievements();
  // รีเฟรชหน้าจอ = ได้สปอว์นตัวเดิม (กันสุ่มใหม่) · ถ้าไม่มีสปอว์นค้างค่อยตั้งเวลาเกิดใหม่
  if (!restoreSpawn()) scheduleSpawn(2200);
