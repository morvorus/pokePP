...
import { SPECIES_ABILITY, ABILITY_DEFS } from './abilities-data.js';
...
function abilityFor(id) {
  if (_abilityCache[id] !== undefined) return _abilityCache[id];
  const key = SPECIES_ABILITY[id];
  let ab = key ? ABILITY_DEFS[key] : null;
  if (key && !ab) {   // มีชื่อความสามารถแต่ยังไม่มีคำอธิบาย — สร้างชื่อสวยๆ จาก identifier
    const nm = key.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
    ab = { name: nm, desc: 'ความสามารถประจำตัว' };
  }
  _abilityCache[id] = ab || null;
